...
"""
Fetch 'Jiu Tang Shu' (舊唐書) from Chinese Wikisource.

Principles (per user requirement):
- Separate by source: outputs go to ./sources/wikisource_jiutangshu/...
...
Output:
  sources/wikisource_jiutangshu/raw/舊唐書_卷006.wikitext
  sources/wikisource_jiutangshu/items/舊唐書_卷006.jsonl
...
def main() -> None:
    ...
    ap.add_argument("--volumes", default="6-7", help="Volume range, e.g. 6-8 or comma list like 6,7")
    ap.add_argument(
        "--out-root",
        default=os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "sources",
            "wikisource_jiutangshu",
        ),
    ...
    for vol in vols:
        ...
        for i, it in enumerate(base_items, start=1):
            rows.append(
                {
                    "id": f"wikisource_jiutangshu_{vol:03d}_{i:05d}",
                    "source": "wikisource",
                    "work": WORK,
                    "volume": f"{vol:03d}",
