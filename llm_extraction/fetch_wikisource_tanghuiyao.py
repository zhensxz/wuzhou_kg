...
"""
Fetch 'Tang Hui Yao' (唐會要) from Chinese Wikisource.

Principles (per user requirement):
- Separate by source: outputs go to ./sources/wikisource_tanghuiyao/...
...
Output:
  sources/wikisource_tanghuiyao/raw/唐會要_卷001.wikitext
  sources/wikisource_tanghuiyao/items/唐會要_卷001.jsonl
...
Note:
- On Chinese Wikisource, Tang Hui Yao volume pages are titled like "唐會要/卷001" (3-digit padding).
"""
...
def main() -> None:
    ...
    for vol in vols:
        ...
        for i, it in enumerate(base_items, start=1):
            rows.append(
                {
                    "id": f"wikisource_tanghuiyao_{vol:03d}_{i:05d}",
                    "source": "wikisource",
                    "work": WORK,
                    "volume": f"{vol:03d}",
